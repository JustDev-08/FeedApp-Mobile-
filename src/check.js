...
import {apiUrlCheck} from '../config.json'
function CheckPage() {
	...
	function sendApi() {
		...
		if (url != '') {
			axios.post(apiUrlCheck ,data)
			.then(res => {
				console.log(res.data.CanConnect);
				let can = res.data.CanConnect
				if (can) {
					setStatus('You can!! connect ')
				}
				else {
					setStatus('You cant!! connect' )
				}
				setWait(false)
			}).catch(e => {
	...
	return (
		<View >
			...
			<ButtonWait wait={wait} pressFunc={sendApi} />
